...
Generations = 8000  # number of GA generations
...
def crossover(x1, x2):
    ...
    p = myPRNG.random()
    if p < crossOverRate:  # crossover rate  with proba p
        if crossOverPt < d / 2:
...
def mutate(x):
    ...
    d = len(x)  # dimensions of solution x
    i = myPRNG.randint(1, d - 1)  # notice I choose the crossover point so that at least 1 element of parent is copied
    p = myPRNG.random()  # random prob p

    if p < mutationRate:  # when prob > p
        if x[i] == 0:  # mutate 0 to 1
            x[i] = 1
        else:  # mutate 1 to 0
            x[i] = 0
...
# insertion step
def insert(pop, kids, k):
    # always keep the best k solutions
    for i in range(k):
        if kids[i] < pop[i]:
            del kids[i]
            kids.insert(i, pop[i])
...
for j in range(Generations):
    ...
    Population = insert(Population, Offspring, 3)
...
print(min(summaryFitness(Population)))
